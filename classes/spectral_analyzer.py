...
class SpectralAnalizer():
    ...
    def stftSynth(self, mY, pY) :
        ...
        out = torch.istft(polar, (mY.shape[0] - 1) * 2, hop_length=self.hop_size, win_length=(mY.shape[0] - 1) * 1, window=torch.hamming_window((mY.shape[0] - 1) * 1), return_complex=False, onesided=True, center=True, normalized=False)
